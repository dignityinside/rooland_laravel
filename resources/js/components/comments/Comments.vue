<template>
    <div class="bg-white py-1 px-4 rounded-lg">
        <template v-if="reply">
        ...
        <template v-else>
            ...
            <p class="mt-4" v-else>No comments</p>
            <a
                href="#"
                class="btn btn-light btn-link btn-block mb-4"
                @click.prevent="loadMore"
                v-if="meta.current_page < meta.last_page"
            >
                Show more
            </a>
            <new-comment :endpoint="endpoint" v-if="user.authenticated"></new-comment>
        </template>
    </div>
</template>
